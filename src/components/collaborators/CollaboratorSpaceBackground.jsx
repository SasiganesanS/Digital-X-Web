...
export default function CollaboratorSpaceBackground() {
  ...
  const layer3Y = useTransform(scrollYProgress, [0, 1], ["0px", "-420px"]); // Uranus Planet

  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-[#050609]">
      
      {/* ── BASE DARK SPACE ATMOSPHERE ── */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#030407] via-[#050B12] via-[#07101C] to-[#040508]" />

      {/* Cyan Teal & Brand Red Cosmic Ambient Glows */}
      <div
        className="absolute top-[6%] right-[8%] w-[850px] h-[520px] rounded-full opacity-35 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(6,182,212,0.25) 0%, rgba(227,29,46,0.12) 55%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[40%] left-[6%] w-[950px] h-[650px] rounded-full opacity-30 blur-[150px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(20,184,166,0.22) 0%, rgba(6,182,212,0.1) 60%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[76%] right-[12%] w-[800px] h-[550px] rounded-full opacity-25 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(6,182,212,0.2) 0%, rgba(10,12,20,0.1) 60%, transparent 80%)",
        }}
      />
        ...
        {FULLPAGE_STARS.map((star) => (
          <motion.div
            ...
            style={{
              ...
              height: `${star.size}px`,
              boxShadow: star.glow
                ? "0 0 8px rgba(255, 255, 255, 0.95), 0 0 14px rgba(6, 182, 212, 0.85)"
      ...
      >
        ...
        >
          <div className="relative w-28 h-28 sm:w-40 sm:h-40 lg:w-48 lg:h-48 drop-shadow-[0_0_35px_rgba(6,182,212,0.45)]">
            <CanvasPlanet
              src={BigMeteorImg}
        ...
        {/* Giant Meteor 2 (Mid-Right Edge ~ 48% - Staggered away from Uranus views) */}
        <motion.div
        ...
        {!shouldReduceMotion &&
          FULLPAGE_COMETS.map((comet) => (
            ...
            >
              {/* Flame Tail */}
              <div className="h-[2.5px] w-[220px] bg-gradient-to-r from-transparent via-[#06B6D4]/80 to-white rounded-full shadow-[0_0_12px_rgba(6,182,212,0.85)] flex-shrink-0" />
              {/* Meteor Head */}
              <div className="w-3 h-3 rounded-full bg-white shadow-[0_0_16px_#FFFFFF,0_0_28px_rgba(6,182,212,1)] -ml-1 z-10 flex-shrink-0" />
            </motion.div>
          ))}
      </motion.div>

      {/* ── LAYER 3: FOREGROUND (URANUS PLANET VIEWS FOR COLLABORATORS PAGE) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: shouldReduceMotion ? 0 : layer3Y }}
      >
        {/* ── URANUS VIEW 1: Main Cyan Ice Giant (Hero Top-Right Outer Edge ~ 2%) ── */}
        <motion.div
        ...
        >
          <div className="relative w-44 h-44 sm:w-60 sm:h-60 lg:w-72 lg:h-72">
            <CanvasPlanet
              src={UranusImg}
              alt="Uranus Planet - Cyan Ice Giant View"
              className="w-full h-full object-contain drop-shadow-[0_0_55px_rgba(6,182,212,0.6)]"
            />
          </div>
        </motion.div>

        {/* ── URANUS VIEW 2: Mid-Page Orbital Perspective (Mid Page Outer-Left Edge ~ 42%) ── */}
        <motion.div
        ...
        >
          <div className="relative w-40 h-40 sm:w-56 sm:h-56 lg:w-64 lg:h-64">
            <CanvasPlanet
              src={UranusImg}
              alt="Uranus Planet - Orbital View"
              className="w-full h-full object-contain drop-shadow-[0_0_45px_rgba(20,184,166,0.65)]"
            />
          </div>
        </motion.div>

        {/* ── URANUS VIEW 3: Lower Horizon View (Lower Right Outer Edge ~ 82%) ── */}
        <motion.div
        ...
        >
          <div className="relative w-36 h-36 sm:w-48 sm:h-48 lg:w-56 lg:h-56">
            <CanvasPlanet
              src={UranusImg}
              alt="Uranus Planet - Horizon View"
