...
export default function ServicesSpaceBackground() {
  ...
  const layer2Y = useTransform(scrollYProgress, [0, 1], ["0px", "-260px"]); // Asteroids, Galaxy & Satellites
  const layer3Y = useTransform(scrollYProgress, [0, 1], ["0px", "-420px"]); // Neptune Planets
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-[#030407]">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#020306] via-[#05070E] via-[#070B16] to-[#030407]" />

      {/* Deep Sapphire Blue & Brand Red Cosmic Ambient Glows */}
      <div
        className={`absolute top-[4%] right-[2%] w-[320px] sm:w-[850px] h-[260px] sm:h-[520px] rounded-full opacity-40 ${isMobile ? "blur-[25px]" : "blur-[140px]"} pointer-events-none`}
        style={{
          background: "radial-gradient(ellipse at center, rgba(59,130,246,0.28) 0%, rgba(227,29,46,0.14) 55%, transparent 80%)",
        }}
      />
      <div
        className={`absolute top-[36%] left-[2%] w-[350px] sm:w-[950px] h-[300px] sm:h-[650px] rounded-full opacity-35 ${isMobile ? "blur-[30px]" : "blur-[150px]"} pointer-events-none`}
        style={{
          background: "radial-gradient(ellipse at center, rgba(30,58,138,0.32) 0%, rgba(59,130,246,0.14) 60%, transparent 80%)",
        }}
      />
      <div
        className={`absolute top-[70%] right-[4%] w-[300px] sm:w-[800px] h-[250px] sm:h-[550px] rounded-full opacity-30 ${isMobile ? "blur-[25px]" : "blur-[140px]"} pointer-events-none`}
        style={{
          background: "radial-gradient(ellipse at center, rgba(59,130,246,0.24) 0%, rgba(10,12,20,0.1) 60%, transparent 80%)",
        }}
      />
        ...
        {(isMobile ? FULLPAGE_STARS.slice(0, 35) : FULLPAGE_STARS).map((star) => (
          <motion.div
            ...
            style={{
              ...
              height: `${star.size}px`,
              boxShadow: star.glow
                ? "0 0 12px rgba(255, 255, 255, 1), 0 0 20px rgba(59, 130, 246, 0.9)"
      ...
      {/* ── LAYER 2: MID BACKGROUND (ALTERNATING METEOR, GALAXY, SATELLITE & ASTEROID) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: disableParallax ? 0 : layer2Y }}
      >
        {/* ITEM 2 (Upper-Left ~ 22%): Burning Fiery Plasma Meteor */}
        <motion.div
        ...
        {/* ITEM 3 (Mid-Right ~ 38%): Photorealistic 3D Deep Blue Spiral Galaxy */}
        <motion.div
        ...
        {/* ITEM 5 (Lower-Right ~ 68%): Photorealistic 3D Space Satellite */}
        <motion.div
        ...
        {/* ITEM 6 (Bottom-Left ~ 82%): Exploding Fiery Asteroid */}
        <motion.div
        ...
        {!shouldReduceMotion &&
          FULLPAGE_COMETS.map((comet) => (
            ...
            >
              {/* Flame Tail */}
              <div className="h-[2.5px] w-[220px] bg-gradient-to-r from-transparent via-[#3B82F6]/85 to-white rounded-full shadow-[0_0_14px_rgba(59,130,246,0.9)] flex-shrink-0" />
              {/* Meteor Head */}
              <div className="w-3.5 h-3.5 rounded-full bg-white shadow-[0_0_18px_#FFFFFF,0_0_30px_rgba(59,130,246,1)] -ml-1 z-10 flex-shrink-0" />
            </motion.div>
          ))}
      </motion.div>

      {/* ── LAYER 3: FOREGROUND (2 NEPTUNE PLANET VIEWS - TOP RIGHT & MID LEFT) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: disableParallax ? 0 : layer3Y }}
      >
        {/* ITEM 1 (Top-Right ~ 3%): Neptune Planet View 1 */}
        <motion.div
        ...
        >
          <div className="relative w-48 h-48 sm:w-64 sm:h-64 lg:w-76 lg:h-76">
            <CanvasPlanet
              src={NeptuneImg}
              alt="Neptune Planet - Sapphire Blue View"
              className="w-full h-full object-contain drop-shadow-[0_0_55px_rgba(59,130,246,0.65)]"
            />
          </div>
        </motion.div>

        {/* ITEM 4 (Mid-Left ~ 52%): Neptune Planet View 2 */}
        <motion.div
        ...
        >
          <div className="relative w-44 h-44 sm:w-60 sm:h-60 lg:w-68 lg:h-68">
            <CanvasPlanet
              src={NeptuneImg}
              alt="Neptune Planet - Orbital View"
              className="w-full h-full object-contain drop-shadow-[0_0_50px_rgba(30,58,138,0.7)]"
            />
          </div>
        </motion.div>
      </motion.div>
