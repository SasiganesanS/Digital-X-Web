...
const FULLPAGE_COMETS = [
  ...
  { id: 2, top: "29%", left: "-15%", delay: 4.5, duration: 2.5, angle: 35 },
  { id: 3, top: "54%", left: "-12%", delay: 8.5, duration: 3.0, angle: 28 },
  { id: 4, top: "77%", left: "-8%", delay: 12.5, duration: 2.7, angle: 36 },
];

export default function CareersSpaceBackground() {
  ...
  const layer3Y = useTransform(scrollYProgress, [0, 1], ["0px", "-420px"]); // Venus Planet
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-[#050609]">
      ...
      {/* Warm Golden Amber & Brand Red Cosmic Ambient Glows */}
      <div
        className="absolute top-[6%] right-[8%] w-[850px] h-[520px] rounded-full opacity-35 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(245,158,11,0.25) 0%, rgba(227,29,46,0.12) 55%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[40%] left-[6%] w-[950px] h-[650px] rounded-full opacity-30 blur-[150px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(234,179,8,0.22) 0%, rgba(245,158,11,0.1) 60%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[76%] right-[12%] w-[800px] h-[550px] rounded-full opacity-25 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(245,158,11,0.2) 0%, rgba(10,12,20,0.1) 60%, transparent 80%)",
        }}
      />
        ...
        {FULLPAGE_STARS.map((star) => (
          <motion.div
            ...
            style={{
              ...
              height: `${star.size}px`,
              boxShadow: star.glow
                ? "0 0 8px rgba(255, 255, 255, 0.95), 0 0 14px rgba(245, 158, 11, 0.85)"
      ...
      >
        ...
        >
          <div className="relative w-28 h-28 sm:w-40 sm:h-40 lg:w-48 lg:h-48 drop-shadow-[0_0_35px_rgba(245,158,11,0.45)]">
            <CanvasPlanet
              src={BigMeteorImg}
        ...
        {!shouldReduceMotion &&
          FULLPAGE_COMETS.map((comet) => (
            ...
            >
              {/* Flame Tail */}
              <div className="h-[2.5px] w-[220px] bg-gradient-to-r from-transparent via-[#F59E0B]/80 to-white rounded-full shadow-[0_0_12px_rgba(245,158,11,0.85)] flex-shrink-0" />
              {/* Meteor Head */}
              <div className="w-3 h-3 rounded-full bg-white shadow-[0_0_16px_#FFFFFF,0_0_28px_rgba(245,158,11,1)] -ml-1 z-10 flex-shrink-0" />
            </motion.div>
          ))}
      </motion.div>

      {/* ── LAYER 3: FOREGROUND (VENUS PLANET VIEWS FOR CAREERS PAGE) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: shouldReduceMotion ? 0 : layer3Y }}
      >
        {/* ── VENUS VIEW 1: Main Golden Atmosphere (Hero Top-Right Outer Edge) ── */}
        <motion.div
        ...
        >
          <div className="relative w-44 h-44 sm:w-60 sm:h-60 lg:w-72 lg:h-72">
            <CanvasPlanet
              src={VenusImg}
              alt="Venus Planet - Golden Atmosphere View"
              className="w-full h-full object-contain drop-shadow-[0_0_55px_rgba(245,158,11,0.6)]"
            />
          </div>
        </motion.div>

        {/* ── VENUS VIEW 2: Mid-Page Orbital Perspective (Mid Page Outer-Left Edge) ── */}
        <motion.div
        ...
        >
          <div className="relative w-40 h-40 sm:w-56 sm:h-56 lg:w-64 lg:h-64">
            <CanvasPlanet
              src={VenusImg}
              alt="Venus Planet - Orbital View"
              className="w-full h-full object-contain drop-shadow-[0_0_45px_rgba(234,179,8,0.65)]"
            />
          </div>
        </motion.div>

        {/* ── VENUS VIEW 3: Lower Horizon View (Lower Right Outer Edge) ── */}
        <motion.div
        ...
        >
          <div className="relative w-36 h-36 sm:w-48 sm:h-48 lg:w-56 lg:h-56">
            <CanvasPlanet
              src={VenusImg}
              alt="Venus Planet - Horizon View"
              className="w-full h-full object-contain drop-shadow-[0_0_40px_rgba(245,158,11,0.55)]"
            />
          </div>
        </motion.div>
      </motion.div>
