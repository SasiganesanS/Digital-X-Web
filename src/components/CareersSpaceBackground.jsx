...
export default function CareersSpaceBackground() {
  ...
  const layer2Y = useTransform(scrollYProgress, [0, 1], ["0px", "-260px"]); // Black Hole, Asteroids & Astronaut
  const layer3Y = useTransform(scrollYProgress, [0, 1], ["0px", "-420px"]); // Venus Planets

  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-[#030407]">
      
      {/* ── BASE DARK SPACE ATMOSPHERE ── */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#020306] via-[#080608] via-[#0F0A0D] to-[#030407]" />

      {/* Warm Golden Amber Cosmic Ambient Glows */}
      <div
        className="absolute top-[4%] right-[2%] w-[850px] h-[520px] rounded-full opacity-40 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(245,158,11,0.28) 0%, rgba(227,29,46,0.14) 55%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[36%] left-[2%] w-[950px] h-[650px] rounded-full opacity-35 blur-[150px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(217,119,6,0.28) 0%, rgba(245,158,11,0.12) 60%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[70%] right-[4%] w-[800px] h-[550px] rounded-full opacity-30 blur-[140px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(245,158,11,0.24) 0%, rgba(10,12,20,0.1) 60%, transparent 80%)",
        }}
      />
        ...
        {FULLPAGE_STARS.map((star) => (
          <motion.div
            ...
            style={{
              ...
              height: `${star.size}px`,
              boxShadow: star.glow
                ? "0 0 12px rgba(255, 255, 255, 1), 0 0 20px rgba(245, 158, 11, 0.9)"
      ...
      {/* ── LAYER 2: MID BACKGROUND (ALTERNATING BLACK HOLE, ASTEROID, METEOR & ASTRONAUT) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: shouldReduceMotion ? 0 : layer2Y }}
      >
        {/* ITEM 2 (Upper-Left ~ 22%): Interstellar Glowing Black Hole */}
        <motion.div
        ...
        {!shouldReduceMotion &&
          FULLPAGE_COMETS.map((comet) => (
            ...
            >
              <div className="h-[2.5px] w-[220px] bg-gradient-to-r from-transparent via-[#F59E0B]/85 to-white rounded-full shadow-[0_0_14px_rgba(245,158,11,0.9)] flex-shrink-0" />
              <div className="w-3.5 h-3.5 rounded-full bg-white shadow-[0_0_18px_#FFFFFF,0_0_30px_rgba(245,158,11,1)] -ml-1 z-10 flex-shrink-0" />
            </motion.div>
          ))}
      </motion.div>

      {/* ── LAYER 3: FOREGROUND (2 VENUS PLANET VIEWS - TOP RIGHT & MID LEFT) ── */}
      <motion.div
        className="absolute inset-0 w-full h-full"
        style={{ y: shouldReduceMotion ? 0 : layer3Y }}
      >
        {/* ITEM 1 (Top-Right ~ 3%): Venus Planet View 1 */}
        <motion.div
        ...
        >
          <div className="relative w-48 h-48 sm:w-64 sm:h-64 lg:w-76 lg:h-76">
            <CanvasPlanet
              ...
              className="w-full h-full object-contain drop-shadow-[0_0_55px_rgba(245,158,11,0.65)]"
            />
          </div>
        </motion.div>

        {/* ITEM 4 (Mid-Left ~ 52%): Venus Planet View 2 */}
        <motion.div
        ...
        >
          <div className="relative w-44 h-44 sm:w-60 sm:h-60 lg:w-68 lg:h-68">
            <CanvasPlanet
              ...
              className="w-full h-full object-contain drop-shadow-[0_0_50px_rgba(234,179,8,0.7)]"
            />
          </div>
        </motion.div>
      </motion.div>
